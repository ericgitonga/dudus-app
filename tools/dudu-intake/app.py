...
def parse_lay_report(pdf_bytes):
    ...
    title is the report's own H1 (the common name, as authored) — possibly wrapped across
    several visual lines, which get joined with a space; every subsequent bold heading starts a
    new section, with regular-weight text collected as that section's body until the next
    heading. Returns error=None on success, or a human-readable reason parsing failed.
    """
